@extends('includes.master')

@section('content')

<!-- ======= Hero Section ======= -->
<section id="hero">
  <div class="hero-container">
    <h1>Project</h1>
    <h2>Solo - Sumber</h2>
    <a href="#about" class="btn-scroll scrollto" title="Scroll Down"><i class="bx bx-chevron-down"></i></a>
  </div>
</section><!-- End Hero -->

<!-- ======= About Me Section ======= -->
<section id="about" class="about">
  <div class="container">
    <div class="section-title">
      <span>Project Solo</span>
      <h2>Project Solo</h2>
      <!--p>Sit sint consectetur velit quisquam cupiditate impedit suscipit alias</p-->
    </div>

    <div class="row" style="padding-bottom: 70px;">
      <div class="col-lg-6 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <img src="assets/img/detail/solo/2.jpg" style="width: 550px;">
      </div>
      <div class="col-lg-6 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <div class="col-lg-12 d-flex flex-column align-items-stretch">
          <div class="content ps-lg-6 d-flex flex-column justify-content-center">
            <div class="row" style="text-align: justify;">
              <h2 style="text-align: center;">RUANG TAMU</h2><hr>
              <p>Mengusung konsep antik dan modern dengan <b><i>classic style</i></b> maskulin dalam proyek ini menjadi pilihan saya. Sang pemilik rumah sangat menyukai ornament dan simbol-simbol budaya jawa sehingga tampilan kokoh dan tahan lama menjadi acuan saya untuk menghadirkan nuansa yang diinginkan.</p>
              <br>
              <br>
              <p><i><b>"Good design means that anything which is good by it self will go with other things"</b></i> (Jean Risom, <b><i>Industrial designer</i></b>)</p>
              <br>
              <br>
              <p>Mengkombinasikan unsur antik seperti wayang dan ukiran 3D batik di dinding dengan unsur seperti modern sofa <b><i>orange</i></b> dan meja bundar kecil menciptakan ruang lebih harmonis dan tidak terkesan penuh atau sesak. Pemilihan material yang didominasi kayu jati Blora dengan <b><i>finishing coating</i></b> PU berwarna coklat seperti pada panel dinding, menimbulkan kesan kokoh dan maskulin.</p>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="row" style="padding-bottom: 70px;">
      <div class="col-lg-4 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <div class="col-lg-12 d-flex flex-column align-items-stretch">
          <div class="content ps-lg-6 d-flex flex-column justify-content-center">
            <div class="row" style="text-align: justify;">
              <h2 style="text-align: center;">RUANG KELUARGA</h2><hr>
              <p><b><i>"Dare to be different"</i></b></p>
              <br>
              <br>
              <p><i><b>"Design is one of the very few fields beda what difference are celebrated"</b></i>(Yves Behar, <i><b>Industrial designer</b></i>)</p>
              <br>
              <br>
              <p>Dalam penataan ruangan tidak ada hukum baku yang mengharuskan seluruh furnitur seragam dan berwarna sama. Padu padan furnitur berbeda warna sah-sah saja. Apa yang diperhatikan adalah keharmonisan dan keseimbangan ruangan secara keseluruhan. </p>
              <br>
              <br>
              <p>Pemilihan aksesoris yang tepat akan membuat ruangan terlihat lebih menarik. Aksesoris dengan nuansa daerah tertentu dapat menjadi fokus tersendiri pada ruangan ini.</p>
            </div>
          </div>
        </div>
      </div>
      <div class="col-lg-8 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <img src="assets/img/detail/solo/5.jpg" style="width: 750px;">
      </div>
    </div>

    <div class="row" style="padding-bottom: 70px;">
      <div class="col-lg-9 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <img src="assets/img/detail/solo/3.jpg" style="width: 820px;">
      </div>
      <div class="col-lg-3 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <div class="col-lg-12 d-flex flex-column align-items-stretch">
          <div class="content ps-lg-6 d-flex flex-column justify-content-center">
            <div class="row" style="text-align: justify;">
              <h2 style="text-align: center;">KAMAR TIDUR UTAMA</h2><hr>
              <p>Posisi tempat tidur yang bersandar pada dinding panel yang tinggi membuat kamar utama ini tampak lebih besar, <b><i>get the position right</i></b> dengan memutuskan <b><i>layout</i></b> yang tepat, maka ruang yang tampak kecil bisa terlihat lebih lega.</p>
              <br>
              <br>
              <p><b><i>"The quality of design is important to the well-being of humanity"</i></b> (Robin Bay, <i><b>Designer</b></i>)</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>

  <div class="container">
    <div class="section-title">
      <span>Before - After</span>
      <h2>Before - After</h2>
    </div>

    <div class="row">
      <div class="col-lg-6 d-flex align-items-stretch justify-content-center justify-content-lg-start" style="margin-bottom: 25px;">
        <img src="assets/img/detail/solo/2.jpg" style="width: 550px;">
      </div>
      <div class="col-lg-6 d-flex align-items-stretch justify-content-center justify-content-lg-start" style="margin-bottom: 25px;">
        <img src="assets/img/detail/solo/3.jpg" style="width: 550px;">
      </div>  

      <div class="col-lg-6 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <img src="assets/img/detail/solo/2b.jpg" style="width: 550px;">
      </div>
      <div class="col-lg-6 d-flex align-items-stretch justify-content-center justify-content-lg-start">
        <img src="assets/img/detail/solo/3b.jpg" style="width: 550px;">
      </div>  
    </div>
    
  </div>
</div>
</section><!-- End About Me Section -->

<!-- ======= My Portfolio Section ======= -->
<section id="portfolio" class="portfolio">
  <div class="container">

    <div class="section-title">
      <span>Completed Projects</span>
      <h2>Completed Projects</h2>
      <p>Berikut beberapa proyek yang telah kami selesaikan</p>
    </div>

    <div class="row portfolio-container">
      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/1.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Living Room</p>
          <a href="assets/img/detail/solo/1.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Living Room from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/2.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Sitting Room</p>
          <a href="assets/img/detail/solo/2.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Sitting Room from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/3.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Master Bedroom</p>
          <a href="assets/img/detail/solo/3.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Master Bedroom from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/4.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Reading Corner</p>
          <a href="assets/img/detail/solo/4.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Reading Corner from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/5.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Living Room</p>
          <a href="assets/img/detail/solo/5.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Living Room from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/6.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Living Room</p>
          <a href="assets/img/detail/solo/6.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Living Room from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/10.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Reading Corner</p>
          <a href="assets/img/detail/solo/9.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Reading Corner from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/9.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Reading Corner</p>
          <a href="assets/img/detail/solo/7.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Reading Corner from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 portfolio-item filter-jogja">
        <div class="portfolio-img"><img src="assets/img/detail/solo/8.jpg" class="img-fluid" alt=""></div>
        <div class="portfolio-info">
          <h4>Project Solo - Sumber</h4>
          <p>Master Bedroom</p>
          <a href="assets/img/detail/solo/8.jpg" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="Master Bedroom from Project Solo - Sumber"><i class="bx bx-show"></i></a>
        </div>
      </div>
    </div>
  </div>
</section><!-- End My Portfolio Section -->

@endsection